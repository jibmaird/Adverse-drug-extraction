...
class LSTM(object):
    ...
    """
    def __init__(self, input_dim, hidden_dim, with_batch=True, name='LSTM', is_train=0, zoc_value =1.0, zoh_value=1.0):
        """
        ...
        self.zoc_value=zoc_value
        self.zoh_value=zoh_value
    ...
    def link(self, input):
        ...
        def recurrence(x_t, c_tm1, h_tm1):
            ...
            if (self.zoc_value < 1.0):
              zoc = self.srng.binomial(n=1, p=self.zoc_value, size=(self.hidden_dim,), dtype=theano.config.floatX)

              c_t_test = ((1 - i_t) * c_tm1 + i_t * T.tanh(T.dot(x_t, self.w_xc) +
                   T.dot(h_tm1, self.w_hc) + self.b_c))

              c_t_train = (zoc * c_t_test) + ((1-zoc) *  c_tm1)

              c_t = ifelse(T.neq(self.is_train, 0), c_t_train, c_t_test)
            else:
              c_t = ((1 - i_t) * c_tm1 + i_t * T.tanh(T.dot(x_t, self.w_xc) +
                    T.dot(h_tm1, self.w_hc) + self.b_c))
            ...
            if (self.zoh_value < 1.0):
              zoh = self.srng.binomial(n=1, p=self.zoh_value, size=(self.hidden_dim,), dtype=theano.config.floatX)
              h_t_test = o_t * T.tanh(c_t)
              h_t_train = (zoh * h_t_test) + ((1-zoh) * h_tm1)

              h_t = ifelse(T.neq(self.is_train, 0), h_t_train, h_t_test)
            else:
              h_t = o_t * T.tanh(c_t)
...
def forward_with_bias(observations, transitions, bias, viterbi=False,
            return_alpha=False, return_best_sequence=False):
    assert not return_best_sequence or (viterbi and not return_alpha)
    ...
    def recurrence(obs, previous, transitions):
        previous = previous.dimshuffle(0, 'x')
        obs = obs.dimshuffle('x', 0)
        obs = T.set_subtensor(obs[0],obs[0]*bias)
        if viterbi:
            scores = previous + obs + transitions
            out = scores.max(axis=0)
            if return_best_sequence:
